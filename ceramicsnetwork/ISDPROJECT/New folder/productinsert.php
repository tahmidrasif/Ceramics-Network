          ...
          <div class="buttonpanel">
			...
			<div class="productform">
                      <form enctype="multipart/form-data" action="#" method="post" name="changer">
                                ...
                                <p class="insert">
                                 <label>viewer:</label> <input type="radio" name="viewer" value="ib"><label>International Buyer</label>
                                               <input type="radio" name="viewer" value="lb"><label>Local Buyer</label>
                                </p>
                                <p class="insert">
			   ...
			   if ($_FILES["image"]["error"]  > 0)
				{
				 ...
				 //  echo"<font size = '5'><font color=\"#0CF44A\">SAVED<br>";
				   ...
				   $path="productimage/".$_FILES["image"]["name"];
						...
						/* if(!empty($detail)&&!empty($price)&&!empty($date)&&!empty($status)&&!empty($viewer)&&!empty($company))
						 {
							 $username="root";$password="";$database="ceramics";
							 mysql_connect("localhost",$username,$password);
							 @mysql_select_db($database) or die( "Unable to select database");
							 
								$query= "INSERT INTO product (companyname, detail, price, image, productsts, viewers) VALUES ('".$company."','".$detail."','".$price."','".$path."','".$status."','".$viewer."')";  
